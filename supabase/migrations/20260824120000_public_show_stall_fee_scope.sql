-- A stall fee's appliesTo can now be an array of barn ids (tick several barns
-- for the same fee), not just 'all' or one barn id. public_show_inventory()
-- previously read it with ->>, which text-extracts a JSON array into its
-- literal string form (e.g. '["barnA","barnC"]') instead of returning the
-- array — the public booking page would then treat that whole string as one
-- bogus barn id and never match a real booking, silently dropping the fee.
--
-- Return appliesTo as raw jsonb instead, so an array stays an array. Missing,
-- empty-string, or empty-array values still default to 'all'. Everything else
-- in this function is unchanged from 20260818120000_public_show_extra_stall_fees.sql.
