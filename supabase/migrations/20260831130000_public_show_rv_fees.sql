-- Robert's "RV and Camping Fee Logic Updates" video, task 1: RV pricing now
-- uses the same "Applies to" fee model as stalls (extraRvFees, mirroring
-- extraStallFees) instead of a price typed directly on each RV area. Adds
-- 'extraRvFees' to the public inventory payload and drops the now-obsolete
-- pricingModel/flatRate passthrough on rvAreas (the client derives both the
-- nightly and flat components live from extraRvFees; pricePerNight stays —
-- the admin keeps it synced to the fee-derived rate, same as barns).
-- Everything else is unchanged from 20260831120000_public_show_barn_dates.sql.
...
SELECT jsonb_build_object(
    ...
    'rvAreas', (
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
            'id',                   rv->>'id',
            'name',                 rv->>'name',
            -- Kept in sync with extraRvFees by the admin (same pattern as a
            -- barn's pricePerNight) — the client derives the flat component
            -- live from extraRvFees instead (see flatRateForRvArea).
    ...
    -- Every RV fee — same shape and 'all'/array appliesTo handling as
    -- extraStallFees, just scoped to RV area ids instead of barn ids.
    'extraRvFees', (
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
            'id',        f->>'id',
            'name',      f->>'name',
            ...
            END,
            'amount',    COALESCE(NULLIF(f->>'amount', '')::numeric, 0),
            'unitType',  COALESCE(NULLIF(f->>'unitType', ''), 'per_night'),
            'dueDate',   NULLIF(f->>'dueDate', ''),
            'lateFee',   COALESCE(NULLIF(f->>'lateFee', '')::numeric, 0)
        )), '[]'::jsonb)
        FROM svc, jsonb_array_elements(COALESCE(s->'extraRvFees', '[]'::jsonb)) f
    )
);
$$;
