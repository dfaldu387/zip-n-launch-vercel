-- get_public_booking() and record_stall_booking_payment() picked ONE price for
-- a barn — its Flat stall fees OR its per-night rate, never both — same bug
-- just fixed in src/lib/extraStallFees.js and src/lib/bookingPricing.js. A
-- barn with a $150/night rate PLUS an "all barns" $25 flat installation fee
-- was billed $25 only, the $150/night silently dropped. That's what the
-- public booking-status page's Total (liveTotal) was showing, even though
-- the line items above it (priced client-side, already fixed) showed the
-- right numbers — the two disagreed on the same page.
--
-- record_stall_booking_payment() is what the Stripe webhook uses to
-- reconcile what a booking owes, so this was a real-money bug on live shows.
--
-- Fix: bill count x (nights x per-night rate + flat rate) — both stack, same
-- as barnPerStallTotal() in src/lib/extraStallFees.js. No behavior change for
-- a barn with only one kind of fee; only barns carrying both now price
-- correctly.
       ...
       AND jsonb_array_length(v_booking->'items') > 0 THEN
        ...
        IF v_ordered_total > 0 THEN
            ...
            LOOP
                ...
                SELECT COALESCE(NULLIF(barn->>'pricePerNight', '')::numeric, 0)
                 ...
                 LIMIT 1;
                v_total := v_total + v_grp.n * (v_nights * COALESCE(v_price, 0) + v_flat_rate);
        ...
        IF v_deficit > 0 THEN
            FOR v_item IN SELECT * FROM jsonb_array_elements(v_booking->'items') LOOP
                ...
                v_total := v_total + v_take * (v_nights * v_price + v_flat_rate);
            END LOOP;
        END IF;
