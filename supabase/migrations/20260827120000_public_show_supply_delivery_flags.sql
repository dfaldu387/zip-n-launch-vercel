-- Supplies can now be flagged Pre-Show Delivery (delivered to stalls before
-- the show, paid in advance) and/or Delivered at Show (sold as a walk-up item
-- during the show) — see HousingGroundsManagerPage.jsx SupplyItemCard. The
-- public pages need both flags to decide what to offer where: the walk-up
-- order page (QuickSupplyOrderPage) only lists a pre-delivery-only item once
-- the show's move-in date has passed, at which point it's sold as a walk-up
-- item just like anything flagged Delivered at Show. Everything else in this
-- function is unchanged from 20260824120000_public_show_stall_fee_scope.sql.
